 ...
 *   3. Per-record ECDSA signature verification
 */
async function verifyIntegrity(pool) {
  ...
  try {
    ...
    // Verify location records: recompute hash + check ECDSA signature
    for (const record of locationRecords) {
      ...
      try {
        const key = ec.keyFromPublic(record.public_key, 'hex');
        const valid = key.verify(record.record_hash, Buffer.from(record.signature, 'hex'));
...
async function runBenchmark(numRecords, numRuns = 10) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  MYSQL BENCHMARK: ${numRecords.toLocaleString()} records, ${numRuns} runs`);
  ...
  for (let run = 0; run < numRuns; run++) {
    ...
    const result = await verifyIntegrity(pool);
  ...
  const results = {
    system: 'MySQL (JOIN + per-record ECDSA)',
    numRecords,
